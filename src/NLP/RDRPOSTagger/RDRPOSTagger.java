...
public class RDRPOSTagger
{
    ...
    public String tagEnSentence(HashMap<String, String> FREQDICT, String sentence, boolean useAlternativeTokenizer)
            throws IOException
    {
        StringBuilder sb = new StringBuilder();
        ...
        List<WordTag> wordtags = InitialTagger.EnInitTagger4Sentence(FREQDICT, line,useAlternativeTokenizer);
    ...
    public void tagEnCorpus(HashMap<String, String> FREQDICT, String inRawFilePath, boolean useAlternativeTokenizer)
            throws IOException
    {
        BufferedReader buffer = new BufferedReader(new InputStreamReader(
                new FileInputStream(new File(inRawFilePath)), "UTF-8"));
        BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(
                new FileOutputStream(inRawFilePath + ".TAGGED"), "UTF-8"));
        for (String line; (line = buffer.readLine()) != null;) {
            bw.write(tagEnSentence(FREQDICT, line,useAlternativeTokenizer) + "\n");
        }
        buffer.close();
        bw.close();
    }
    ...
    public List<String> finalTags(List<WordTag> initTags) {

        List<String> finalTagList = new ArrayList<>();
        int size = initTags.size();

        for (int i = 0; i < size; i++) {
            FWObject object = Utils.getObject(initTags, size, i);
            Node firedNode = findFiredNode(object);
            if (firedNode.depth > 0) {

                //    sb.append(wordtags.get(i).word + "/" + firedNode.conclusion + " ");

                finalTagList.add(firedNode.conclusion);
            } else
                ...
                finalTagList.add(initTags.get(i).tag);
            }
        }
